...
generate_raw_report() {
echo "📊 Ideon Scraping Internal - Daily Summary - Generated By Claude Code"
...
"   • \(.name) (\(.section)) - due \(.due_on) → \(.assignee)"
...
echo ""
echo "📋 WORKLOAD DISTRIBUTION (Active Tasks)"
echo ""
...
curl -s "${API_BASE}/projects/${PROJECT_GID}/tasks?opt_fields=gid,name,assignee.name,completed,memberships.section.name&limit=100" \
  ...
  jq -r '
    ...
    group_by(.assignee.name // "⚠️ Unassigned") |
    map({
      assignee: (.[0].assignee.name // "⚠️ Unassigned"),
      count: length,
...
if [ "$POST_CLICKUP" = true ]; then
  ...
  echo ""
  echo "📤 Posting to ClickUp #Scraping channel..."
  echo ""
  ...
  if echo "$CLICKUP_RESPONSE" | jq -e '.id' > /dev/null 2>&1; then
    echo "✅ Posted to ClickUp #Scraping channel successfully!"
  else
    echo "❌ Failed to post to ClickUp:"
